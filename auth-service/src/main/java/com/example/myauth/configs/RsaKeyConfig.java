...
public class RsaKeyConfig {
    ...
    public RsaKeyConfig(
            ...
            @Value("${security.jwt.private-key-path:classpath:keys/private_key.pem}") String privateKeyPath,
    ...
    public PrivateKey jwtPrivateKey() {
        try {
            return KeyFactory.getInstance("RSA")
                    .generatePrivate(new PKCS8EncodedKeySpec(loadKeyBytes(privateKeyPath, "PRIVATE KEY")));
        } catch (Exception ex) {
            throw new IllegalStateException("Unable to create JWT private key.", ex);
        }
    }
